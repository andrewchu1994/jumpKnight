...
//	private projShooter2 shooter10;
	...
	void Start () {
		...
		shooter = FindObjectOfType<projShooter> ();
		shooter2 = FindObjectOfType<projShooter2> ();
	...
	void OnTriggerEnter2D(Collider2D other){
		...
		if (stop == false) {
			if (other.tag == "Player") {
				...
				Instantiate (obj, new Vector3(shooter.transform.position.x,shooter.transform.position.y,0), Quaternion.identity);
				Instantiate (obj2, new Vector3(shooter2.transform.position.x,shooter2.transform.position.y,0), Quaternion.identity);
